...
negative — meaning direct is statistically significantly better than
pivot for Thai, though the effect is small. Vietnamese is still not
distinguishable from zero.

We rewrote the paragraph honestly to reflect the new asymmetry. Please rate:

1. Does the paragraph accurately report the new numbers?
2. Does the interpretation match what the data actually say?
...
we hit 324 usable pairs per language after filtering empty direct outputs
and length/character-set sanity checks). Judge model: CometKiwi-DA
(reference-free, GPU inference, bootstrap over pair-level scores with 10k
resamples).
...
- **Interpretation** — does the "small but significant Thai gap" claim
  match a CI of [-0.039, -0.010]? Is "~5% of direct score" a fair way to
